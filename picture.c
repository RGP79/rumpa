...
void png_map(Map *M){
  ...
  row_pointers_out = create_image((M->rozmiar_x)*32, (M->rozmiar_y)*32);
  init_png_file((M->rozmiar_x)*32, (M->rozmiar_y)*32, row_pointers_out);
  for(int i = M->rozmiar_y-1; i>= 0; i--)
    for(int j = 0; j < M->rozmiar_x; j++)
      copy_tile(j*32, i*32, 32, 32, row_pointers_out, row_pointers, ktory_numer(M->plansza[(M->rozmiar_y-1)-i][j]));
  write_png_file("map.png", (M->rozmiar_x)*32, (M->rozmiar_y)*32, color_type, bit_depth, row_pointers_out);
